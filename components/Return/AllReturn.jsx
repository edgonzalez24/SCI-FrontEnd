...
import { getReturns, deleteReturn } from '../../store/actions/returnAction';
...
const All_Returns = ({ returns, pages, getReturns }) => {
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                              ...
                              </thead>
                              {
                                ...
                                <tr>
                                  ...
                                  <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">
                                    { `${item.id_student.name} ${item.id_student.lastname}` }
                                  </td>
                                  <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">
                                    {item.id_student.academic_degree}
                                  </td>
                                  <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">
                                    {item.id_book.title_book}
                                  </td>
                                  <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">
                                  ...
                                  <td className="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium flex justify-between ">
                                    ...
                                    >
                                      ...
                                      <DialogActions>
                                        ...
                                        <Button onClick={() => removeData(item._id)} color="primary" className="focus:outline-none" autoFocus>
                                          Confirmar
                                        </Button>
                                      </DialogActions>
                                    </Dialog>
...
const mapStateToProps = (state) => ({
    ...
    pages: state.return.pages,
});

const mapDispatchToProps = (dispatch) => ({
    getReturns: (currentPage) => dispatch(getReturns(currentPage))
});
export default connect(mapStateToProps, mapDispatchToProps)(All_Returns);